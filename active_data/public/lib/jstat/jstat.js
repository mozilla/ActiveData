...
var BetaDistribution = ContinuousDistribution.extend({
	...
	_pdf: function(x, give_log){
		...
		if (x == 0){
			...
			/*a == 1 */
			return (give_log) ? Math.log(b) : b;    // R_D_val(b)
		}
		if (x == 1){
			if (b > 1){
				// R_D__0
			...
			/* b == 1 */
			return (give_log) ? Math.log(a) : a;   // R_D_val(a)
		}
		if (a <= 2 || b <= 2){
...
var StudentTDistribution = ContinuousDistribution.extend({
	...
	_pnt: function(t, lower_tail, log_p){
		...
		x = t * t;
		rxb = dof / (x + dof);
		/* := (1 - x) {x below} -- but more accurately */
		x = x / (x + dof);
		/* in [0,1) */
		if (x > 0.){/* <==>  t != 0 */
			lambda = del * del;
