    ...
    def test_range_packing2(self):
        where = {"and": [
            {"eq": {
                ...
                "build.type": "debug",
                "result.test": "browser/devtools/debugger/test/browser_dbg_variables-view-02.js"
            }},
        ...
        self.assertEqual(result, {"and": [
            {"term": {"run.suite": "mochitest-devtools-chrome"}},
            {"term": {"build.platform": "linux"}},
            {"term": {"run.chunk": 1}},
            {"term": {"result.test": "browser/devtools/debugger/test/browser_dbg_variables-view-02.js"}},
            {"term": {"build.type": "debug"}},
            {"term": {"build.branch": "mozilla-inbound"}},
            {"range": {"build.date": {"gte": Date("23 APR 2015").unix, "lt": Date("25 APR 2015").unix}}}
        ]})
